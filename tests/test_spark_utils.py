...
def test_check_path_auto_conversion ( spark_session: SparkSession ):
	...
	check_path = df_check_path ( out_path )
	df_save ( test_df, check_path )

	# The out_path must exist
	assert_that ( out_path, "Output path exists" ).exists ()

	# So, let's try the loading
	loaded_df = df_load ( check_path, spark_session )
...
def test_checkpoint_manager_repartition ( 
	...
	saved_df = df_save ( test_df, ckpt_path, target_partition_size = target_partition_size )
