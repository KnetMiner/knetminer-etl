...
## The `df_xxx()` functions

These functions concern saving and loading DataFrames and are useful to build incremental workflows
that can be restarted from intermediate data.

These functions are purposefully abstract, all a client needs to know is that the path associated
...
def df_save ( 
	...
	new_partitions = df_new_partition_size ( size, target_partition_size )
...
def df_load ( path_or_df: str | DataFrame, spark: SparkSession ) -> DataFrame:
	"""
	Loads a dataframe which was saved as an intermediate by :func:`df_save`.
	...
	When this is a path, it is passed to :func:`df_path` to automatically retrieve 
	...
	if isinstance ( path_or_df, DataFrame ):
		log.debug ( f"df_load() got a DataFrame as input, returning it" )
		return path_or_df
	
	if not spark:
		raise ValueError ( 
			"df_load(): a SparkSession is required when loading from a path." 
		)

	path = df_path ( path_or_df )
	log.info ( f"Loading DataFrame from {path}" )
...
def df_check_path ( base_path: str ) -> str:
	...
	with loading and saving methods, which automatically invokes :fun:`df_path`.
...
def df_path ( df_path: str ) -> str:
	"""
	Returns the base path that was used to save a DataFrame checkpoint 
	(a parquet directory in the current implementation).

	This is used in :func:`df_load` and :func:`df_save`, to automatically
	retrieve the base path from a check path.

	This is useful in frameworks like SnakeMake, ie, if you use :func:`df_check_path` 
...
def df_new_partition_size ( estimated_df_size: int, target_partition_size: int = 256 * 1024 ** 2 ) -> int:
	"""
	Choose a reasonable no of partitions to be used by :func:`df_save`, so each output file is 
